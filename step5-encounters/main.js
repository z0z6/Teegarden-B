...
// ============================================================
// KROK 5: RASY, WALKA I SCENY FABULARNE
// ============================================================
...
window.addEventListener('keydown', (e) => {
  ...
  if (scene5 && playerState.alive) encounters.trigger(scene5);
  if (e.code === 'Digit0' && playerState.alive) encounters.startDemo();
});
...
function updateStory(delta) {
  ...
  labels.update(npcs.list.filter((n) => n.alive).map((n) => ({
    id: n.id,
    position: n.group.position,
    title: `${n.callsign} · ${n.raceName}`,
    sub: n.side === 'ally' ? 'sojusznik' : 'wróg',
    color: n.side === 'ally' ? '#4dd6a0' : '#ff5a4a',
  })));
}
