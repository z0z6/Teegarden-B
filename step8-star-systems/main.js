...
const npcs = createNpcManager(scene, combat, playerProxy, { warp, weapons });
...
function updateStory(delta) {
  ...
  labels.update(npcs.list.filter((n) => n.alive && !n.hidden && !n.arriving).map((n) => ({
    ...
    sub: n.side === 'neutral' ? `fałda: ${n.warpName}` : `${n.side === 'ally' ? 'sojusznik' : 'wróg'} · ${n.weaponName}`,
...
function swapSystem(id) {
  ...
  for (const n of npcs.hostiles()) npcs.remove(n);
  renderSystemUi();
}
