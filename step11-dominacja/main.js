...
const economy = createEconomy({
  ...
  fieldsFor: (id, sp) => generateFields(id, sp), saveKey: `teegarden-b.dominacja.v1.${PLAYER_RACE0}`,
...
function updateDominion(delta) {
  ...
  if (scanT <= 0) {
    scanT = 0.5;
    if (playerState.alive) economy.scan(shipGroup.position, SENSOR_RANGE * (1 + 0.08 * (playerStats.attrs.sensors - 5)));
...
function updateEconomy(delta) {
  ...
  mining.last = economy.playerMine(delta, shipGroup.position, sh.forward, cap, alive && (mining.held || mining.touch), _beamFrom);
...
if (!urlMission) comms.say({
  sender: 'Dominacja', sub: 'kampania', color: '#ffd36b',
  text: matchMedia('(pointer: coarse)').matches
    ? 'Dominacja: rasy dzielą sektor między siebie. KOP — wydobycie, panel na dole — przemysł. Sygnały „nieznane złoże” prowadzą do nowych pól. Zbieraj, rozbudowuj, zbrój się — i sięgnij po połowę sektora.'
    : 'Dominacja: rasy dzielą sektor między siebie. Kopiesz (T), budujesz (P), sygnały „nieznane złoże” prowadzą do nowych pól, a mapa strategiczna (M) pokazuje, kto co trzyma, i prowadzi dyplomację. Zbieraj, rozbudowuj, zbrój się — i sięgnij po połowę sektora.',
  ttl: 14,
});
